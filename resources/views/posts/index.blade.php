...
@section('content')
    ...
    @foreach($posts as $post)
        <img alt="Zdjęcie profilowe użytkownika {{$post->user->firstname}} {{$post->user->lastname}}"
             src="{{url('')}}/{{$post->user->profile->image}}">
        <h2><a href="{{url('/users')}}/{{$post->user->id}}">{{$post->user->firstname}} {{$post->user->lastname}}</a>
        </h2>
        <p>{{$post->created_at}}</p>
        <p>{{$post->content}}</p>
        <a href="{{url('/posts')}}/{{$post->id}}">{{count($post->reactions)}} reakcji, {{count($post->comments)}}
            komentarzy</a>
